...
const BlogPage = () => {
    ...
    const fetchBlog = () => {
        axios.get(import.meta.env.VITE_SERVER_DOMAIN + `/get-blog?blog_id=${blog_id}`)
        .then(async ({ data: { blog } }) => {

            blog.comments = await fetchComments({ blog_id: blog._id, setParentCommentCountFun: setTotalParentCommentsLoaded })
            setBlog(blog)
            
            axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/search-blogs", { category: blog.category._id, limit: 6, eliminate_blog: blog_id })
            .then(({ data }) => {
                    ...
                    <div className="max-w-[900px] center py-10 max-lg:px-[5vw]">
...

                        {
                            similarBlogs != null && similarBlogs.length ?
                                <>
                                    <h1 className="text-2xl mt-14 mb-10 font-medium">Similar Articles</h1>
